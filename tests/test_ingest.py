...
VERWACHTE_KOLOMMEN = {
    ...
    "leertraject", "opleidingscode", "vorig_onderwijs_niveau",
    "vorig_onderwijs_graad", "geboortedatum", "leeftijd",
}
...
def test_parse_ro(pad: Path) -> None:
    ...
    assert df["brin"].is_not_null().any()
...
def test_parse_ro_lege_dataframe_als_geen_data(tmp_path: Path) -> None:
    """parse_ro op een leeg bestand geeft lege DataFrame met correct schema."""
